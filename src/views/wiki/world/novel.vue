    ...
    <div>
      <div>
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item :to=" { path: '/world/details', query: {wid:world.value.id} }">{{world.name}}</el-breadcrumb-item>
...
import {  getWorldByName } from "@/api/wiki/world";
...
function handleFllow(){
  ...
  addFllow(world.value.id).then(response => {
    isFllow.value=true
...
function handleFllowClose(){
  ...
  fllowClose(world.value.id).then(response => {
    isFllow.value=false
...
function getAllWorldComment() {
  queryParams.value.wid=world.value.id;
...
function handleIsFllow(){
  ...
  if(isLogin==true) {
    getInfoByWid(wname.value).then(response => {
